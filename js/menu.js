...
function initDoneList(){
	...
	rui.h.query(db, 'select * from t_plan_day_done order by id desc', function(res){
...
function doneItemHandler(event){
	...
	rui.h.query(db, 'select * from t_plan_day_todo where id=' + todoId, function(res){
		if(res.rows.length > 0){
			var data = res.rows.item(0);
			
			rui.h.query(db, 'select max(id) mid from t_plan_day_done', function(res1){
				...
				rui.h.update(db, 'insert into t_plan_day_done (id, plan_title, plan_content) values (' + (id+1) + ', "' + data.plan_title + '", "' + data.plan_content + '")');
				rui.h.update(db, 'delete from t_plan_day_todo where id=' + todoId);
			});
		}
	});
}