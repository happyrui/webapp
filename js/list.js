...
mui.plusReady(function(){
	...
	window.addEventListener('swiperight', function(){
		rui.h.indexPage().evalJS("opMenu();");
	});
	
	// 查看详情
	rui.on('#todolist li', 'tap', function(){
		rui.h.fire('detail', 'detailItem', {id:$(this).data('id')});
	});
	
	// 完成
	rui.on('.doneBtn', 'tap', function(){
		var $li = $(this).parent().parent();
		var id = $li.data('id');
		$li.remove();
		showList($('#todolist'));
		
		rui.h.fire('menu', 'doneItem', {todoId:id});
		return false;
	});
	
	// 长按
	rui.on('#todolist li', 'longtap', function(){
		tapId = $(this).data('id');
		rui.h.pop();
	});
	
	// 删除
	rui.on('.delli', 'tap', delItem);
...
function initHelp(){
	...
	if(help == null){
		...
		rui.h.insertItem('help','notfirst');
	}
...
function initList(){
	...
	rui.h.query(db, 'select * from t_plan_day_todo order by id desc', function(res){
...
function addItemHandler(event){
	// 主界面按钮修改
	rui.h.indexPage().evalJS("hideBackBtn();");
	
	var title = event.detail.title;
	var content = event.detail.content ? event.detail.content : '暂无内容！';
	
	rui.h.query(db, 'select max(id) mid from t_plan_day_todo', function(res){
		var id = (res.rows.item(0).mid) ? res.rows.item(0).mid : 0;
		rui.h.update(db, 'insert into t_plan_day_todo (id, plan_title, plan_content) values (' + (id+1) + ', "' + title + '", "' + content + '")');
...
function delItem(){
	if(tapId){
		rui.h.update(db, 'delete from t_plan_day_todo where id=' + tapId);
		rui.h.pop();
		initList();
	}
}